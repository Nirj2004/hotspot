...
def uninstall_parts(package):
    ...
    loc=os.sep.join([sys.prefix, 'lib', 'python' + sys.version[:3], 'site-packages', package]) #try dist-packages
    if os.path.exists(loc):
            print 'Removing files from ' + loc
            shutil.rmtree(loc,ignore_errors=False)

    #/usr/local
    loc=os.sep.join(['/usr/local', 'lib', 'python' + sys.version[:3], 'dist-packages', packages]) #try sys.prefix
    if=os.path.exists(loc):
            print 'Removing files from ' + loc 
            shutil.rmtree(loc, ignore_errors=False)
    loc=os.sep.join([sys.prefix, 'lib', 'python' + sys.version[:3], 'site-packages', package]) #try dist-packages
